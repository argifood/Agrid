...
public class DAO {
	...
	public class Role extends Element{
		...
		Map<String, List<DAO.Asset>> assetPerRoles = new HashMap<>();
		Map<DAO.Asset, List<DAO.Task>> tasksPerAsset = new HashMap<>();
	}
	...
	public class Asset extends Element{
		...
		List<Task> assetAssociatedTasks = new ArrayList<>();
	...
	public static DAO createFromJSON(JSONObject daoJson) {
		...
		JSONObject methodPlugin =
				null;
		try {
		methodPlugin = daoJson.getJSONObject("uma:MethodLibrary")
	...
	private void fillRoleMaps() {
		// asset associated roles
		...
		for(String key : assets.keySet()) {
			...
			// Get role-to-role association
			for (Task t : tasks.values()) {
				if (t.output.equals(a.id)) {
					for (String id : t.performersID) {
						Role r = roles.get(id);
						for (String id2 : t.performersID) {
							if (r.associatedRoles.contains(roles.get(id2).camelCaseName)) continue;
							r.associatedRoles.add(roles.get(id2).camelCaseName);
							r.associatedRoleIDs.add(id2);
						}
					}
				}
			}
			// get asset-to-roles association
			for(Task t : tasks.values()) {
				if(t.output.equals(a.id)) {
					for(String id : t.performersID) {
						if(!a.assetAssociatedRoles.contains(roles.get(id)))
							a.assetAssociatedRoles.add(roles.get(id));
					}
				}
			}

			// get asset-to-tasks association
			for(Task t : tasks.values()) {
				if(t.output.equals(a.id) || t.mandatoryInput.equals(a.id)) {
					a.assetAssociatedTasks.add(t);
				}
			}
		}
		// get assets that contain associated roles
		for(Role r : roles.values()){
			for(String associateId : r.associatedRoleIDs){
				// find assets that associate both
				List<Asset> associatedAssets = new ArrayList<>();
				List<String> associatedAssetNames = new ArrayList<>();
				for(Asset a : assets.values()){
					if(a.assetAssociatedRoles.contains(r) && a.assetAssociatedRoles.contains(roles.get(associateId))) {
						if(associatedAssetNames.contains(a.name)) continue;
						associatedAssets.add(a);
						associatedAssetNames.add(a.name);
					}
				}
				r.assetPerRoles.put(associateId, associatedAssets);
			}
		}
	}
	...
	private void printParticipants(PrintWriter writer) {
		writer.print("/**\r\n" + 
				...
				" */\r\n" + 
				"participant Entity identified by entityKey {\r\n" + 
				"  o String entityKey\r\n" + 
				"}\r\n");
